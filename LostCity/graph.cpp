...
void Graph::incrementA(int delta) { a_ += delta; buildCurve(*font_); }
void Graph::incrementB(int delta) { b_ += delta; buildCurve(*font_); }
void Graph::incrementC(int delta) { c_ += delta; buildCurve(*font_); }
...
void Graph::buildCurve(sf::Font& font)
{
    ...
    // Fenêtre fixe : toujours -10 à 10 en X et Y
    xMin_ = -10.f; xMax_ = 10.f;
    yMin_ = -10.f; yMax_ = 10.f;

    float scaleX = gw / (xMax_ - xMin_);
    float scaleY = gh / (yMax_ - yMin_);
